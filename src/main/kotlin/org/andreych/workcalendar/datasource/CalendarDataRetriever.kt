    ...
    suspend fun getData(): Pair<String?, List<YearData>> {
        ...
        val deferred = withContext(Dispatchers.IO) {
            return@withContext async {
                val response: String? = restTemplate.getForObject(URL_TEMPLATE, String::class.java, accessToken)
                val value = objectMapper.readValue(response, Array<YearData>::class.java)
                return@async Pair(response, value?.asList() ?: emptyList())
            }
        }
